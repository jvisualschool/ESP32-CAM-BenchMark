...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    ...
    if (avg_frame_time > 0) {
      global_fps = 1000.0f / avg_frame_time;
    } else {
      global_fps = 0.0f;
    }
...
static esp_err_t fps_handler(httpd_req_t *req) {
  char buf[32];
  snprintf(buf, sizeof(buf), "%.1f", global_fps);
  httpd_resp_set_type(req, "text/plain");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, buf, strlen(buf));
}

static esp_err_t benchmark_handler(httpd_req_t *req) {
  httpd_resp_set_type(req, "text/html");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
...
<head>
  <meta charset="utf-8">
  <title>ESP32-CAM Benchmark Tool</title>
  <style>
...
<body>
  <div class="container">
    ...
    <div class="controls">
      <div class="control-group">
        <label>Resolution (Frame Size)</label>
      ...
      <div class="control-group">
        <label>XCLK Frequency (Boost FPS)</label>
        <select id="xclk" onchange="updateXCLK(this.value)">
          ...
          <option value="20" selected>20 MHz (Default)</option>
          <option value="22">22 MHz</option>
          <option value="24">24 MHz (High FPS)</option>
        </select>
      </div>
      <div class="control-group">
        <label>JPEG Quality (0-63, Lower = better quality)</label>
        <input type="range" id="quality" min="10" max="63" value="12" onchange="updateConfig('quality', this.value); document.getElementById('qv').innerText=this.value;">
        <div style="font-size:0.8em;text-align:right" id="qv">12</div>
      </div>
      <div class="control-group">
        <label>Actions</label>
        <button class="btn" onclick="startStream()">Restart Stream</button>
  ...
  <script>
    ...
    function startStream() {
      // Use correct port logic (Stream is usually ServerPort + 1)
      const loc = window.location;
      let streamPort = parseInt(loc.port) + 1;
      if (loc.port === "80" || loc.port === "") streamPort = 81;
      streamImg.src = `${loc.protocol}//${loc.hostname}:${streamPort}/stream?t=${Math.random()}`;
      statusMsg.innerText = "Stream Started.";
    }

    function updateConfig(varName, val) {
      fetch(`/control?var=${varName}&val=${val}`)
        .then(response => {
           if(response.ok) statusMsg.innerText = `${varName} updated to ${val}`;
           else statusMsg.innerText = `Failed to update ${varName}`;
        });
    }

    function updateXCLK(val) {
      fetch(`/xclk?xclk=${val}`)
        .then(response => {
           if(response.ok) statusMsg.innerText = `XCLK updated to ${val} MHz`;
           else statusMsg.innerText = `Failed to update XCLK`;
        });
    }
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &benchmark_uri);
  }
